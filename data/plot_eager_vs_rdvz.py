...
label = ["eager-enabled", "eager-disabled"]
coll = "reduce_scatter"
...
for idx, collective in enumerate([f"{coll}_2", f"{coll}_3"]):
    ...
    for buffer_size in buffer_sizes:
        ...
        if idx == 1:
            plt.plot(x_values, subset[key], label=f"{label[idx]}-{buffer_size} MB", 
                    linestyle=linestyles[i], color=colors[idx], marker=markers[i])
...
if not os.path.exists(f"eager_v_rdvz/{coll}/{impl}/"):
    os.makedirs(f"eager_v_rdvz/{coll}/{impl}/")
plt.savefig(f"eager_v_rdvz/{coll}/{impl}/plot_{buffer_size}_MB.pdf", bbox_inches='tight')
