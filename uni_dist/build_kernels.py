...
def build():
    ...
    def _cpp_extention_load_helper(name, sources, extra_cuda_flags, extra_include_paths, extra_link_flags):
        ...
        return cpp_extension.load(
            ...
            extra_cflags=['-O3', '-std=c++17', '-D__HIP_ROCclr__', 
                          ...
                          '-DUSE_ROCM', '-DUSE_MPI', '-x hip', "-I"+mpi4py.get_include()],
    ...
    if torch.version.hip is not None:
        extra_include_paths=[os.path.join(str(pathlib.Path(__file__).parent.absolute()), "include"), 
                             "/opt/rocm-6.2.4/include"]
    else:
        extra_include_paths=[]

    mpi4py_incl_path = mpi4py.get_include()
    extra_include_paths += mpi4py_incl_path
